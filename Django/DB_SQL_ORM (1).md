...
3. 잔고는 오름차순, 나이는 내림차순으로 10명?
      ...
      ```python
   # orm
   # 이렇게 하면 안됨! User.objects.order_by('balance').order_by('-age')
   ```
# 결국 -age로만 정렬하는 꼴이기 때문. 정답은 아래!
...
```sql
SELECT country, MAX(balance) AS max_balance, MIN(balance) AS min_balance FROM users_user GROUP BY country;
```
